...
public class DiagnosisWorkService {
  ...
  @Transactional public DiagnosisWorkResponse create(SaveDiagnosisWorkRequest request) { modules.requireEnabled(MODULE); Company company = tenant.currentCompany(); return response(works.save(DiagnosisWork.create(company, tenant.currentUser(), request.workDate(), request.enterpriseName().trim(), request.address().trim(), members(company, request.participantIds())))); }
  @Transactional public DiagnosisWorkResponse update(Long id, SaveDiagnosisWorkRequest request) { modules.requireEnabled(MODULE); DiagnosisWork work = requireWork(id); work.update(request.workDate(), request.enterpriseName().trim(), request.address().trim(), members(work.getCompany(), request.participantIds())); return response(work); }
