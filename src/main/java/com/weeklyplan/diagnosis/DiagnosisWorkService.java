...
public class DiagnosisWorkService {
  ...
  @Transactional public DiagnosisWorkResponse create(SaveDiagnosisWorkRequest request) { modules.requireEnabled(MODULE); Company company = tenant.currentCompany(); return response(works.save(DiagnosisWork.create(company, tenant.currentUser(), request.workDate(), request.enterpriseName().trim(), request.county().trim(), request.diagnosisTime().trim(), request.diagnosisRound(), text(request.enterpriseContact()), text(request.enterpriseContactPhone()), members(company, request.participantIds())))); }
