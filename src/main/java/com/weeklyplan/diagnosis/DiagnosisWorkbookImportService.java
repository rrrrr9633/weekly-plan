...
public class DiagnosisWorkbookImportService {
  ...
  public List<ImportedDiagnosisRow> parse(MultipartFile file) {
    ...
    try (InputStream input = file.getInputStream(); Workbook workbook = new XSSFWorkbook(input)) {
      ...
      for (int rowIndex = header.rowIndex + 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
        ...
        rows.add(new ImportedDiagnosisRow(date, enterprise, countyOnly(text(row, header.county)), text(row, header.diagnosisTime), round(text(row, header.round)), text(row, header.contact)));
      }
      return rows;
  ...
  private Header locateHeader(Sheet sheet) {
    for (Row row : sheet) {
      int date = -1, enterprise = -1, county = -1, diagnosisTime = -1, round = -1, contact = -1;
      for (Cell cell : row) { String value = formatter.formatCellValue(cell).trim(); if (value.contains("时间") || value.contains("日期")) date = cell.getColumnIndex(); if (value.contains("企业名称") || value.equals("企业")) enterprise = cell.getColumnIndex(); if (value.contains("县区") || value.contains("地址")) county = cell.getColumnIndex(); if (value.contains("诊断时间")) diagnosisTime = cell.getColumnIndex(); if (value.contains("第几次") || value.contains("入企")) round = cell.getColumnIndex(); if (value.contains("服务商联系人") || value.equals("联系人")) contact = cell.getColumnIndex(); }
      if (date >= 0 && enterprise >= 0) return new Header(row.getRowNum(), date, enterprise, county, diagnosisTime, round, contact);
    }
  ...
  private record Header(int rowIndex, int date, int enterprise, int county, int diagnosisTime, int round, int contact) {}
}
